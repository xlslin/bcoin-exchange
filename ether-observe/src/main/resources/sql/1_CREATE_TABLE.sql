...
(
  ...
  STATUS                VARCHAR(20) NOT NULL COMMENT '状态(CSH:初始化、ZBSJTB:准备数据同步、SJTBZ:数据同步中、WYZ:未验证、QKYZYX:区块验证有效、QKYZWX:区块验证无效)',
  CREATE_TIME           TIMESTAMP NULL COMMENT '创建时间',
  MODIFY_TIME           TIMESTAMP NULL COMMENT '修改时间',
  PRIMARY KEY (ID)
);
...
(
  ...
  STATUS                VARCHAR(20) NOT NULL COMMENT '处理状态(WCL:未处理、CLZ:处理中、SUCCESS:充值成功、FAIL:充值失败)',
  CREATE_TIME           TIMESTAMP NULL COMMENT '创建时间',
  MODIFY_TIME           TIMESTAMP NULL COMMENT '修改时间',
  PRIMARY KEY (ID)
);
...
(
  ...
  TX_HASH               VARCHAR(100) COMMENT '交易hash',
  STATUS                VARCHAR(20) NOT NULL COMMENT '处理状态(WCL:未处理、CLZ:处理中、SUCCESS:提现成功、FAIL:提现失败)',
  CREATE_TIME           TIMESTAMP NULL COMMENT '创建时间',
  MODIFY_TIME           TIMESTAMP NULL COMMENT '修改时间',
  PRIMARY KEY (ID)
);
...
(
  ID                    CHAR(32) NOT NULL COMMENT 'id',
  BLOCK_CHAIN_ID        CHAR(32) NOT NULL COMMENT '区块id',
  BLOCK_NUMBER          BIGINT COMMENT '区块号',
  BLOCK_HASH            VARCHAR(100) NOT NULL COMMENT '区块hash',
  TX_HASH               VARCHAR(100) NOT NULL COMMENT '交易hash',
  TX_STATUS             VARCHAR(20) NOT NULL COMMENT '交易状态(WCL:未处理、CLZ:处理中、YCL:已处理)',
  CREATE_TIME           TIMESTAMP NULL COMMENT '创建时间',
  MODIFY_TIME           TIMESTAMP NULL COMMENT '修改时间',
  PRIMARY KEY (ID)
);
ALTER TABLE TRANSACTION_TEMP COMMENT 'ETH交易临时表';
